/**
 * 숨겨진 누락 데이터 수정 스크립트
 * - 기존 파싱 로직의 HTML 구조 인식 한계로 놓친 캐릭터들 추가
 * - 개선된 파싱 로직: spanText === strongText 조건 완화
 *
 * 사용법:
 *   npx tsx scripts/fix-hidden-missing.ts --dry-run     # 미리보기
 *   npx tsx scripts/fix-hidden-missing.ts               # 실제 실행
 */
...
const HIDDEN_MISSING: Array<{ patchId: number; characters: string[] }> = [
  { patchId: 3242, characters: ['미르카', '히스이'] },
  { patchId: 3149, characters: ['테오도르'] },
  {
    patchId: 3098,
    characters: [
      '나타폰',
  ...
  { patchId: 2916, characters: ['아이작'] },
  { patchId: 2885, characters: ['레니'] },
  { patchId: 2765, characters: ['다르코'] },
  { patchId: 2592, characters: ['버니스', '에키온'] },
  { patchId: 2364, characters: ['알론소'] },
  { patchId: 2331, characters: ['레니'] },
  { patchId: 2237, characters: ['알론소'] },
  { patchId: 2201, characters: ['바바라'] },
  { patchId: 2188, characters: ['클로에'] },
  { patchId: 2102, characters: ['아르다'] },
  { patchId: 1942, characters: ['루크'] },
  { patchId: 1900, characters: ['이안'] },
  { patchId: 1812, characters: ['바냐', '테오도르'] },
  { patchId: 1727, characters: ['다니엘'] },
];
...
async function main(): Promise<void> {
  ...
  console.log('숨겨진 누락 데이터 수정 시작...\n');
  if (dryRun) {
    console.log('*** DRY RUN 모드 - 실제 저장하지 않음 ***\n');
  }

  // 총 항목 수 계산
  const totalItems = HIDDEN_MISSING.reduce((acc, p) => acc + p.characters.length, 0);
  console.log(`수정 대상: ${totalItems}개 항목 (${HIDDEN_MISSING.length}개 패치)\n`);
