...
export function setValidCharacters(chars: Set<string>): void {
...
export async function parsePageContent(page: Page): Promise<ParsedCharacter[]> {
          ...
          if (skillMatch && !headerText.includes('→')) {
            ...
            if (!isSkillHeader) {
              if (headerText.includes('→')) {
                ...
                // 자손 li 중 → 포함 항목이 있으면 계층형 라벨 (stat prefix)로 처리
                const hasArrowDescendants = Array.from(topLi.querySelectorAll('li')).some(
                  function (li) {
                    const s = li.querySelector('span');
                    return s ? (s.textContent?.includes('→') ?? false) : false;
                  }
                );
                if (hasArrowDescendants) {
                  statLabelPrefix = headerText;
                } else {
                  // 설명형 변경사항
                  currentChanges.push({
                    _type: 'description',
            ...
            if (descSpan) {
              ...
              if (!isDescSkillHeader) {
                if (descText.includes('→')) {
                  ...
                  if (descNumMatch) {
                    const rawDescStat = descNumMatch[1].trim();
                    currentChanges.push({
                      _type: 'numeric',
                      target: currentTarget,
                      stat: statLabelPrefix
                        ? (statLabelPrefix + ' ' + rawDescStat).trim()
                        : rawDescStat,
    ...
    .map((char) => ({
      ...
      changes: char.changes.map((change): Change => {
        ...
        if (rawChange._type === 'description') {
          ...
          // added/removed는 항상 설명형으로 처리
          if (processed.changeCategory === 'added' || processed.changeCategory === 'removed') {
            const description = `${rawChange.stat} ${rawChange.before} → ${rawChange.after}`
              .replace(/\s+/g, ' ')
              .trim();
            return {
              target: rawChange.target,
          ...
          // mechanic: before/after에 숫자가 없는 경우만 설명형 (e.g. "고정 피해 → 스킬 피해")
          // 숫자가 있으면 괄호 내 수치 변경 등으로 numeric으로 처리 (e.g. "(공격력의 108%)")
          if (processed.changeCategory === 'mechanic') {
            const hasDigits = /\d/.test(rawChange.before) || /\d/.test(rawChange.after);
            if (!hasDigits) {
              const description = `${rawChange.stat} ${rawChange.before} → ${rawChange.after}`
                .replace(/\s+/g, ' ')
                .trim();
...
if (
  scriptName.endsWith('parse-balance-changes.ts') ||
  scriptName.endsWith('parse-balance-changes.js')
) {
